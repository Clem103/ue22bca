...
class RobotControl:
    ...
    def follow_path_advanced(self, rb, spd, max_dist, filt_l, filt_r, filt_f):
        ...
        while state:
            ...
            distance_right = filt_r.median_value(raw_dist_r, 3)
            distance_left = filt_l.median_value(raw_dist_l, 3)
            distance_front = filt_f.moving_avg(raw_dist_f, 4)

            distance_from_center_left = abs(
                1 / 2 * self.track_width - distance_left)  # Distance au centre basée sur le capteur gauche
            distance_from_center_right = abs(
                1 / 2 * self.track_width - distance_right)  # Distance au centre basée sur la capteur droite
            distance_from_center = (distance_from_center_right + distance_from_center_left) / 2

            v_lat = 1 / 2 * ((old_d_left - distance_left) + (
                    distance_right - old_d_right))  # Comptée positive lorsque l'on s'approche à gauche
            ...
            if left or middle or right:
                ...
                if seq_line == 'g' and seq_line!=seq_line_old:
                    rb.set_speed(spd*(1+Kp_line), spd*(1-Kp_line))
                elif seq_line == 'd' and seq_line!=seq_line_old:
                    rb.set_speed(spd*(1-Kp_line), spd*(1+Kp_line))
                else:
                    seq_line_old = seq_line
            ...
            print('Dist Front :', distance_front)
